...
from sar.sar_filters import lee_filter
...
def test_uniform_image(uniform_linear_image):
    filtered = lee_filter(uniform_linear_image)
    assert np.allclose(filtered.data, uniform_linear_image.data)

def test_output_shape(linear_image):
    filtered = lee_filter(linear_image)
    assert filtered.data.shape == linear_image.data.shape

def test_requires_linear_scale(sample_image):
    with pytest.raises(ValueError):
        lee_filter(sample_image)
